...
;(function() {
  ...
  var _formatOCToJS = function(obj) {
    ...
    if (obj instanceof Function) {
        return function() {
            var args = Array.prototype.slice.call(arguments) //获得obj所有args
            var formatedArgs = _OC_formatJSToOC(args) //args参数_js转换成oc参数
        ...
        return _OC_formatOCToJS(obj.apply(obj, formatedArgs)) //执行的结果，转换成js
      }
    }
    if (obj instanceof Object) { //剩下的是字典
      var ret = {}
      for (var key in obj) {
  ...
  var _methodFunc = function(instance, clsName, methodName, args, isSuper, isPerformSelector) {
    ...
    return _formatOCToJS(ret)//执行的结果，oc类转成js类
  }
  ...
  var _customMethods = {
    __c: function(methodName) {
      ...
      if (slf instanceof Boolean) {//false是nil
        return function() {
          return false
        }
      }
      ...
      if (!slf.__obj && !slf.__clsName) { //不是类或对象..
        throw new Error(slf + '.' + methodName + ' is undefined')
      }
      if (slf.__isSuper && slf.__clsName) { //父类的名字..因为js中也有继承
